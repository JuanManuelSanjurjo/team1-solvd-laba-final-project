...
import RecentlyViewed from "@/app/(side-bar)/(profile)/recently-viewed/components/RecentlyViewed";
...
jest.mock("@/components/cards/CardContainer", () => (props: any) => (
  <div data-testid="card-container">{props.children}</div>
));
...
describe("RecentlyViewed", () => {
  ...
  it("shows skeleton when not hydrated", () => {
    ...
    render(<RecentlyViewed session={sessionProp} />);

    expect(screen.getByTestId("title")).toHaveTextContent("Recently Viewed");
  ...
  it("shows EmptyState and go to /products when hydrated and empty list", () => {
    ...
    fireEvent.click(screen.getByTestId("empty-go-products"));
    expect(push).toHaveBeenCalledWith("/products");
  });

  it("renders CardContainer y Cards when there is products", () => {
    setupStoreMock({
  ...
  it("calls useCleanUpGhostProducts with corrects IDs and it callback calls removeInactiveProducts(userId, ids)", () => {
    ...
    const [idsArg, callback] = (useCleanUpGhostProducts as jest.Mock).mock
      .calls[0];
